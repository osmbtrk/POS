...
if (isset($_SESSION['user'])){
    ...
    if($do=='Manage'){
                        ...
                        <div class="form-group form-group-lg" style="display:flex; flex-direction: row; align-items: center">
                            ...
                            <div class="col-sm-10 col-md-4">
                                <select class="category" id="category" name="category">
                                    ...
                                    <?php
                                        $stmt=$con->prepare("SELECT * FROM categories WHERE parentID=1");
                                        $stmt->execute();
                                        $cats=$stmt->fetchAll();
                                        foreach($cats as $cat){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $search  = $_POST['sea'];
            $check=checkItem("barcode",'article',$search);
            if($check==0){
                $theMsg='<div class="alert alert-danger">sorry this title is not exist</div>';
                redirectHome($theMsg,'back');
            }else{
                    //insert
                    $getItems=$con->prepare("SELECT * FROM article WHERE barcode=?");
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $check=checkItem("barcode",'article',$search2);
            if($check==1){
                $theMsg='<div class="alert alert-danger">sorry this title is not exist</div>';
                redirectHome($theMsg,'back');
            }else{
                    //insert
                    $getItems=$con->prepare("SELECT * FROM article WHERE categoryid=? AND id=?");
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $stmt=$con->prepare("SELECT * FROM article WHERE id=$artid");
            $stmt->execute();
            $cats=$stmt->fetchAll();
            foreach($cats as $cat){
            ...
            if($check<$check1){
                    ...
                    header("refresh:$seconds;url=home.php");
                    ...
                    $stmt->execute();
                    $stm = $con->prepare("UPDATE article SET quantite= ? WHERE id= ?");
