...
<html>
	...
	<body>
		<a class="back" href="javascript: history.back()"><i class='fa fa-chevron-left'></i> Back</a>
		...
		<div id="blue-playlist-container">
			...
			<div id="amplitude-player">
				...
				<div id="amplitude-right">
					...
					INNER JOIN artiste ON artiste.ID=items.artiste WHERE cat_ID=?");
		...
		<script>
			Amplitude.init({
				...
				"songs": [
					...
					$catid=$_GET['pageid'];
                    $getItems=$con->prepare("SELECT items.*,artiste.name AS artiste_name FROM items 
                    INNER JOIN artiste ON artiste.ID=items.artiste WHERE cat_ID=? ORDER BY item_ID ASC");
                    $getItems->execute(array($catid));
					$items =$getItems->fetchAll();
                    foreach($items as $item){?>
					{
