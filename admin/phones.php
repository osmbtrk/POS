...
if (isset($_SESSION['username'])){
    ...
    if($do=='Manage'){
        $stmt=$con->prepare("SELECT phones.*,categories.catname AS cat_name FROM phones 
            INNER JOIN categories ON categories.id=phones.categoryid");
        $stmt->execute();
        $items=$stmt->fetchAll();
        ?>
         <h1 class="text-center">Manage phones</h1>
         <div class="container">
                     ...
                     <?php
                         foreach($items as $item) {
                             echo "<tr>";
                                 ...
                                 echo  "<td>
                                 <a href='phones.php?do=Edit&itemid=" .$item['id']. "' class='btn btn-success'><i class='fa fa-edit'></i> Edit</a>
                                 <a href='phones.php?do=Delete&itemid=" .$item['id']. "' class='btn btn-danger confirm'><i class='fa fa-close'></i> Delete</a>
             ...
             <a href="phones.php?do=Add" class="btn btn-primary"><i class="fa fa-plus"></i> Add item</a>
         </div><?php
    }elseif($do=='Add'){?>
         ...
         <div class="container">
            <form class="form-horizontal" action="?do=Insert" method="Post">
                ...
                <div class="form-group" style="display:flex; flex-direction: row; align-items: center">
                    <label class="col-sm-2 control-label"><b>Camera</b></label>
                    <div class="col-sm-10 col-md-4">
                        <div>
                            <input id="com-yes" type="radio" name="camera" value="0" checked />
                            <label for="com-yes">Yes</label>
                        </div>
                        <div>
                            <input id="com-no" type="radio" name="camera" value="1" />
    ...
    <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $reference  = $_POST['category'].$_POST['name'];
            $formErrors = array();
            ...
            if(empty($formErrors)){
                ...
                $check=checkItem("artname","phones",$name);
                if($check==1){
                    $theMsg='<div class="alert alert-danger">sorry this name is exist</div>';
                    redirectHome($theMsg,'back');
                }else{
                    //insert
                    $stmt= $con->prepare("INSERT INTO phones(artname,quantite,barcode,prix,prixbase,categoryid,couleur,camera,doublesim,reference) VALUES(:name,:quantite,:barcode,:prix,:prixbase,:cat,:couleur,:camera,:doublesim,:reference)");
                    $stmt->execute(array(
                        'name' => $name,
                        'quantite' => $quantite,
                        'barcode' => $barcode,
        ...
        $stmt = $con->prepare("SELECT * FROM phones WHERE id = ? ") ;
         ...
         <div class="container">
            <form class="form-horizontal" action="?do=Update" method="Post">
                ...
                <div class="form-group" style="display:flex; flex-direction: row; align-items: center">
                    ...
                    <div class="col-sm-10 col-md-4">
                        <div>
                            <input id="com-yes" type="radio" name="camera" value="0" <?php if($item['camera']==0){echo 'checked';} ?> />
                            <label for="com-yes">Yes</label>
                        </div>
                        <div>
                            <input id="com-no" type="radio" name="camera" value="1" <?php if($item['camera']==1){echo 'checked';} ?>/>
                            <label for="com-no">No</label>
                        </div>
                    </div>
                </div>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if(empty($formErrors)){
                $stmt = $con->prepare("UPDATE phones SET artname = ?,barcode = ?, quantite = ?, prix = ?, prixbase = ?,categoryid= ?, couleur = ?, ram = ?, stockage = ?,batterie= ?, faceid = ?, emprint = ?, doublesim = ? WHERE id=? ");
                $stmt->execute(array($name,$barcode,$quantite,$prix,$prixbase,$cat,$couleur,$camera,$doublesim,$id));
        ...
        echo "<div class='container'>";
            $itemid= isset($_GET['itemid']) && is_numeric($_GET['itemid']) ? intval($_GET['itemid']) : 0 ;
            $check=checkItem("id","phones",$itemid);
            if ($check>0){
                $stmt=$con->prepare('DELETE FROM phones WHERE id=:id');
